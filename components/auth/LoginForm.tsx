...
import { credentialsSignIn, googleSignIn, githubSignIn } from '@/lib/actions/auth'
...
export default function LoginForm({ registered = false, oauthError = null }: LoginFormProps) {
    ...
    <div className="space-y-5">
      ...
      {oauthError && (
        <div className="rounded-lg bg-amber-50 px-4 py-3 text-sm text-amber-800">
          {oauthErrorMessages[oauthError] ?? 'An error occurred during sign in. Please try again.'}
        </div>
      )}

      <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
        {authError && (
          <p className="rounded-lg bg-red-50 px-4 py-3 text-sm text-red-700">{authError}</p>
        )}
      ...
      <div className="space-y-3">
        ...
        <button
          ...
          className="flex w-full items-center justify-center gap-2 rounded-lg bg-gray-900 px-4 py-2.5 text-sm font-medium text-white hover:bg-gray-700 transition-colors"
        >
